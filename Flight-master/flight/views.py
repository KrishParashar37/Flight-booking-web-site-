...
try:
    ...
    if len(Flight.objects.all()) == 0:
        print("Database is empty. Automatically adding flights...")
        addDomesticFlights()
    ...
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            try:
                ...
                # Send email and WhatsApp for ticket 1
                from capstone.utils import send_notifications
                email_ok = False
                notification_error = ""
                try:
                    email_sent, whatsapp_sent = send_notifications(ticket)
                    email_ok = email_sent
                ...
                if t2:
                    ...
                    # Send email and WhatsApp for ticket 2
                    try:
                        send_notifications(ticket2)
                    except Exception:
                        pass
...
def get_ticket(request):
    ...
    return render(request, 'flight/ticket.html', data)
