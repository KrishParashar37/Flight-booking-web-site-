document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => {
        let fetch1 = fetch(`/flight/ticket/api/${ref1}`).then(response => response.json());
        let fetch2 = ref2 ? fetch(`/flight/ticket/api/${ref2}`).then(response => response.json()) : Promise.resolve(null);
        
        Promise.all([fetch1, fetch2])
        .then(([ticket1, ticket2]) => {
